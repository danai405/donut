...
cl.login(token="EnewjP0y2OeUikXTMy3d.q54JaLG084oPKmYLdpYwpq.7O1O03dlxRKu05gBnZthWTQBv2DpViC5Os8Wxku80dQ=")
cl.loginResult()

ki = LINETCR.LINE() #Zorro
ki.login(token="EnB4d0DEOuS2mLzwCAx4.es8hyE+hUxj3aGoFVGl1Da.5Gxxh5LdLDlXivURY1oTqjyR7776rxGhmJvyd12Stzo=")
ki.loginResult()

kk = LINETCR.LINE() #Sanji
kk.login(token="En1qdZPJSFu3DFHx7dM4.a2yP/qFVWGawGYNQEAbwja.emEM8GV8HOEIJEAMTVKb1yWc+fNZ5wbQqXvSU6vUnqI=")
kk.loginResult()

kc = LINETCR.LINE() #Ussop
kc.login(token="EnM41Zvxp7wUv1nMtSN0.9vtjE887Sod39Kibjxxfia.y7BGGo+xzxKZq7GxltWrQzqfUP5YjBwjQu+96XzA3L4=")
kc.loginResult()

ks = LINETCR.LINE() #Chooper
ks.login(token="EnFCgufEmf7PXLjzbDt0.252bwbxpgTzdqNxoj+WQWa.+LmYm8N6eRh0r/LBmM4BHtq7cUK1aes1BR+68FMRMaQ=")
ks.loginResult()
...
admin=["ub3db3cc8a5db36da5186eb52d14bfaa9","u4f8709f6222b794c1f03e7e04ea20277","u531f68025e1b287f71b64588a8b11875","ub3d561a2c32242e74f2cd53b4d72da29","u6fe16c90ea1b71404b429593539bc7b3","u4d0c7094d063ef17a32a2e0872f9901a","u47c8bcc7bbca8bc0028dfdf00f3efafd"]
...
def bot(op):
          ...
          if wait["Protectgr"] == True:
            if cl.getGroup(op.param1).preventJoinByTicket == False:
              ...
              if op.param2 in admin:
                ...
                try:
                  cl.sendText(op.param1,cl.getContact(op.param2).displayName + "คุณไม่ได้รับสิทธิ์ในการเปิด QR")
                  cl.kickoutFromGroup(op.param1,[op.param2])
                  X = cl.getGroup(op.param1)
                  X.preventJoinByTicket = True
                  cl.updateGroup(X)
                except:
                  random.choice(KAC).sendText(op.param1,random.choice(KAC).getContact(op.param2).displayName + "คุณ ไม่ได้รับสิทธิ์ในการเปิด QR")
    ...
    #--------------- SC Add Admin ---------
            ...
            elif msg.text in ["Adminlist","แอดมิน"]:
              if admin == []:
                  cl.sendText(msg.to,"The stafflist is empty")
              else:
                  cl.sendText(msg.to,"แสดงรายชื่อแอดมิน")
                  mc = "|| ─═हई..तุএച้ට௮..ईह═─ ||================\n\n"
                  for mi_d in admin:
    ...
    #-----------------=Selesai=------------------
              ...
              if msg.from_ in admin:
                if msg.toType == 2:
                    ...
                    if wait["lang"] == "JP":
                        random.choice(KAC).sendText(msg.to,"สถานะ url \nเปิด")
                    else:
                        random.choice(KAC).sendText(msg.to,"สถานะ url \nเปิด")
                else:
                    if wait["lang"] == "JP":
		...
		for ticket_id in n_links:
			if wait["atjointicket"] == True:
              ...
              if msg.from_ in admin:
            ...
            elif msg.text in ["Wkwkwk","Wkwk","Wk","5555","555","55"]:
                msg.contentType = 7
                msg.text = None
                msg.contentMetadata = {
                                     "STKID": "100",
                ...
                ki.sendMessage(msg)
                #kk.sendMessage(msg)
            elif msg.text in ["Hmmm","Hmm","Hm","hmmm","อิบอท","บอท"]:
                msg.contentType = 7
                msg.text = None
                msg.contentMetadata = {
                                     "STKID": "101",
              ...
              if msg.from_ in admin:
                if wait["Protectgr"] == True:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"การป้องกัน QR = On")
                    else:
                        cl.sendText(msg.to,"done")
                else:
                    wait["Protectgr"] = True
              ...
              if msg.from_ in admin:
                if wait["Protectgr"] == False:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"การป้องกัน QR = Off")
                    else:
                        cl.sendText(msg.to,"done")
                else:
              ...
              if msg.from_ in admin:
                if wait["contact"] == True:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"สถานะตรวจสอบ Mid = On")
                    else:
                        cl.sendText(msg.to,"done")
                else:
                    wait["contact"] = True
              ...
              if msg.from_ in admin:
                if wait["contact"] == False:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"สถานะตรวจสอบ Mid = Off")
                    else:
                        cl.sendText(msg.to,"done")
                else:
                    wait["contact"] = False
...
#-----------------------------------------------
         #----------------Fungsi Join Group Start-----------------------#
            elif msg.text in ["กุ๊กๆๆ","binla:on","Binla:On"]: #Panggil Semua Bot
              if msg.from_ in owner:
    ...
    #-------------Fungsi Leave Group Start---------------#
            elif msg.text in ["ชิ้วๆๆ","binla:bey","Binla:Bye"]: #Bot Ninggalin Group termasuk Bot Induk
              if msg.from_ in admin:
                if msg.toType == 2:
                    ginfo = cl.getGroup(msg.to)
                    try:
    ...
    #-------------Fungsi Tag All Finish---------------#
      ...
      #--------------List Group------------
       #------------ Keluar Dari Semua Group------
            elif msg.text in ["Bot out","Binla:out"]: # Keluar Dari Semua Group Yang Di dalem nya  ada bot(Kalo Bot Kalian Nyangkut di Group lain :D)
              if msg.from_ in owner:
                gid = cl.getGroupIdsJoined()
  ...
  #-----------------End-----------
            elif msg.text in ["สวัสดี"]:
                ki.sendText(msg.to,"หวัดดี 􀜁􀅔Har Har􏿿")
                kk.sendText(msg.to,"หวัดดี 􀜁􀅔Har Har􏿿")
                kc.sendText(msg.to,"หวัดดี 􀜁􀅔Har Har􏿿")
...
#-----------------------------------------------
            elif msg.text in ["ปิ๊ง","ปิง","ping"]:
                ki.sendText(msg.to,"ป่อง 􀨁􀄻double thumbs up􏿿􀜁􀅔Har Har􏿿")
                #kk.sendText(msg.to,"ป่อง 􀨁􀄻double thumbs up􏿿􀜁􀅔Har Har􏿿")
...
#-----------------------------------------------
      ...
      #-------------Fungsi Balesan Respon Start---------------------#
            elif msg.text in ["มันคืออะไร","อะไร"]:
                ki.sendText(msg.to,"จะไปรู้เรอะ 􀨁􀅴questioning􏿿")
      ...
      #-------------Fungsi Banned Send Contact Start------------------#
              ...
              if msg.from_ in owner:
                ...
                cl.sendText(msg.to,"กรุณาส่ง contact")
                ki.sendText(msg.to,"กรุณาส่ง contact")
                kk.sendText(msg.to,"กรุณาส่ง contact")
                kc.sendText(msg.to,"กรุณาส่ง contact")
      ...
      #-------------Fungsi Chat ----------------
            elif msg.text in ["อิบอท","บอท","bot","Bot"]:
                 quote = ['เรียกทำไม จะนอน \nดูเวล่ำเวลาบ้าง 😂😂😂.']
      ...
      #-------------Fungsi Bannlist Start------------------#
              ...
              if msg.from_ in admin:
                if wait["blacklist"] == {}:
                    random.choice(KAC).sendText(msg.to,"ไม่มีรายชื่อในบัญชี Blacklist")
                else:
...
#---------------------
        if op.type == 17:
          ...
          random.choice(KAC).sendText(op.param1, "ยินดีต้อนรับเข้าสู่ กลุ่ม" + str(ginfo.name) + "\n By :" + ginfo.creator.displayName)
          #random.choice(KAC).sendText(op.param1,"Budayakan Baca Note !!! yah Ka 😊\nSemoga Betah Kk 😘")
          print "MEMBER HAS JOIN THE GROUP"
        if op.type == 15:
          if op.param2 in Bots:
             return
          random.choice(KAC).sendText(op.param1, "โอกาสหน้าเจอกันใหม่นะจร๊")
